...
with open(args.esdlist, 'r') as f: #get ESDs from csv file
    ...
    for row in reader:
        ...
        if args.exporttype in ['html', 'both']:
            try:
                ...
                if not isError:
                    ...
                    for p in processed:
                        for i in soup.find_all('img'):
                            if p['src'] == i['src']:
                                i['src'] = p['relpath']
                    outpath = os.path.join(args.outpath, esd, '.'.join((esd, 'html')))
        ...
        if args.exporttype in ['pdf','both']:
            outpath = os.path.join(args.outpath, esd, '.'.join((esd, 'pdf')))
            try:
                pdfkit.from_url(link, outpath)
            except OSError:
                print('wkhtmltopdf not found. Please install wkhtmltopdf on this system and make sure it is in you PATH for pdf exports.')
                raise
